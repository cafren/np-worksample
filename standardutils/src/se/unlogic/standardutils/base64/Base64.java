...
public class Base64 {
	...
	public static String encodeFromInputStream(java.io.InputStream inputStream, long streamLength) throws java.io.IOException {

		String encodedData = null;
		Base64.InputStream bis = null;
		try{
			// Set up some useful variables
			byte[] buffer = new byte[Math.max((int)(streamLength * 1.4 + 1), 40)]; // Need max() for math on small files (v2.2.1); Need +1 for a few corner cases (v2.3.5)
			int length = 0;
			int numBytes = 0;

			// Open a stream
			bis = new Base64.InputStream(new java.io.BufferedInputStream(inputStream), Base64.ENCODE);
